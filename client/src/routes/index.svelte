<script context="module">
	...
	export async function preload({ params, query }) {
		let data = await getGameListGames({
...
<script>
	...
	function moveContainer(direction) {
		let rectanglesWidth = document.querySelector('rectangles').offsetWidth - 74 * 2
		...
		if (direction === 'left') {
			counter++
			gameListTranslate = -gameCardWidth * foo * counter - 32 * foo * counter + 74
		} else {
...
<list>
	...
	<section>
		<rectangles flex="justify-between">
			<rectangle cursor="pointer" flex="jusify-center align-center" num="1" on:click={() => moveContainer('right')}>
				<img icon="fit-width" src="icons/arrow-back.svg" alt="" />
			</rectangle>
			<rectangle cursor="pointer" flex="jusify-center align-center" num="2" on:click={() => moveContainer('left')}>
				<img icon="fit-width" src="icons/arrow-forward.svg" alt="" />
			</rectangle>
		</rectangles>
		<container flex="direction-row" style="transform:translateX({gameListTranslate}px)">
