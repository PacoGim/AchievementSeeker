...
<script>
	...
	import { getGameBackground, getGameLogo, getSafeFolderName } from 'services/game.service.js'
	...
	onMount(async () => {
		setFancyBG('#000', '#000', 'skewY(0deg) translateY(-50vh)', 'skewY(0deg) translateY(-50vh)', 0.1)
	...
	<upper>
		<background>
			<logo flex="justify-center align-center" {logoLoaded}>
				<picture>
					<source srcset="http://localhost:4000/images/{getSafeFolderName(game._id, game.name)}/logos/logo.webp" type="image/webp" />
					<source srcset="http://localhost:4000/images/{getSafeFolderName(game._id, game.name)}/logos/logo.png" type="image/png" />
