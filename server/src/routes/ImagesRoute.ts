...
router.get('/:type/:id', async (ctx:ParameterizedContext) => {
	...
	if (type === 'header') {
		...
		if (!fs.existsSync(fullPath)) {
			...
			let game = await getCustomGame({ filter: { _id: id }, project: { appid: 1 } })
		...
		let game = await getCustomGame({ filter: { _id: id }, project: { appid: 1, backgrounds: 1 } })
...
router.get('/achievement/:id/:achievementId/:isGray*', async (ctx:ParameterizedContext) => {
	...
	let game = await getCustomGame({ filter: { _id: id }, project: { 'achievements._id': 1, 'achievements.img': 1, 'achievements.imgGray': 1, appid: 1 } })
